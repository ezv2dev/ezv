...
class SearchHomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('user.list_villa', compact('villa', 'amenities', 'host_language', 'accessibility_features', 'accessibility_features_detail', 'propertyType', 'villaCategory', 'villaFilter'));
    }
    ...
    public function processFilter($villas, $request)
    {
        ...
        if ($filter) {
            ...
            $villaIds = $villa->modelKeys();
            $villa = Villa::with('villaHasFilter')->where('villa.status', 1)
                ->whereIn('villa.id_villa', $villaIds)
                ->whereHas('villaHasFilter', function (Builder $query) use ($filter) {
                    $query->where('id_villa_filter', $filter);
                })
                ->inRandomOrder()->get();
        }
