            ...
            <tbody style="text-align: center;" class="tbody-font">
                @forelse ($allConversationList as $conversation)
                    <tr>
                        ...
                        <td>{{ $conversation->approve_status }}</td>
                        <td>
                            <center>
                            <div class='dropdown'>
                                ...
                                <div class='dropdown-menu dropdown-menu-end font-size-sm' aria-labelledby='dropdown-align-primary'>
                                    <a class='dropdown-item admin-action-dropdown' href="#">View</a>
                                    @if($conversation->approve_status == 'waiting' && in_array(auth()->user()->role->name, ['admin', 'superadmin']))
                                        @if ($conversation->conversation_sender_type == 'user')
                                            <hr>
                                            hit approve user
                                            <form action="{{ route('villa_update_approve_user_message') }}" method="post">
                                                @csrf
                                                <input type="hidden" name="id_message" value="{{ $conversation->id_message }}">
                                                <button class='dropdown-item admin-action-dropdown' type="submit">approve</button>
                                            </form>
                                            <form action="{{ route('villa_update_disapprove_user_message') }}" method="post">
                                                @csrf
                                                <input type="hidden" name="id_message" value="{{ $conversation->id_message }}">
                                                <button class='dropdown-item admin-action-dropdown' type="submit">disapprove</button>
                                            </form>
                                        @else
                                            <hr>
                                            hit approve owner
                                            <form action="{{ route('villa_update_approve_owner_message') }}" method="post">
                                                @csrf
                                                <input type="hidden" name="id_reply" value="{{ $conversation->id_reply }}">
                                                <button class='dropdown-item admin-action-dropdown' type="submit">approve</button>
                                            </form>
                                            <form action="{{ route('villa_update_disapprove_owner_message') }}" method="post">
                                                @csrf
                                                <input type="hidden" name="id_reply" value="{{ $conversation->id_reply }}">
                                                <button class='dropdown-item admin-action-dropdown' type="submit">disapprove</button>
                                            </form>
