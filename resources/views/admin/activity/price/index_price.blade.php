...
<div class="bg-body-light">
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
        <h1 class="flex-grow-1 fs-3 fw-semibold my-2 my-sm-3">{{ $data[0]->name }} Price</h1>
        <nav class="flex-shrink-0 my-2 my-sm-0 ms-sm-3" aria-label="breadcrumb">
          <ol class="breadcrumb">
            <a type="button" class="btn btn-sm btn-alt-primary" href="{{ route('admin_activity_create_price', $data[0]->id_activity) }}">
                <i class="fa fa-plus-circle"></i> Add Data
            </a>
            <input type="hidden" id="id_activity" name="id_activity" value="{{ $data[0]->id_activity }}">
          </ol>
        </nav>
      </div>
    </div>
...
<script>
    ...
    var table = $('#tabel').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax:"/admin/activity/datatable_price/" + $("#id_activity").val(),      
        columns: [
