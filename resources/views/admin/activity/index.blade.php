    ...
    <div class="content content-full">
      <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
        ...
        <nav class="flex-shrink-0 my-2 my-sm-0 ms-sm-3" aria-label="breadcrumb">
          <ol class="breadcrumb">
            {{-- @if($data == 0)
                <a type="button" class="btn admin-adddata-button btn-sm btn-alt-primary" href="{{ route('villa_add_step_one') }}">
                    <i class="fa fa-plus-circle"></i> Add Data
                </a>
            @elseif($data > 0)
                <a type="button" class="btn admin-adddata-button btn-sm btn-alt-primary" href="{{ route('activity_add_step_continue') }}">
...
<script>
    ...
    var table = $('#tabel').dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax:"{{ route('admin_activity_datatable') }}",
        columns: [
