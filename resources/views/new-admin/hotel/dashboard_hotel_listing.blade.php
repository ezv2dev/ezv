...
@section('content_admin')
    ...
    if (Auth::user()->role_id == 1 || Auth::user()->role_id == 2) {
        $hotelName = App\Models\Hotel::select('id_hotel', 'name')->get();
    } else {
        $hotelName = App\Models\Hotel::where('created_by', Auth::user()->id)
            ->select('id_hotel', 'name')
            ->get();
    }
    @endphp
            ...
            <div class="content content-full">
                <div class="row">
                    <div class="col-4 pt-10">
                        @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 2)
                            <h1 class="flex-grow-1 fs-3 fw-semibold ">{{ $hotelName->count() }} Total Listing</h1>
                        @else
                            ...
                            <div class="location-popup-container h-100">
                                @foreach ($hotelName as $item)
                                    <div class="col-lg-12 location-popup-desc-container sugest-list-first lozad"
                                        loading="lazy"
                                        onclick="window.open('{{ route('hotel', $item->id_hotel) }}', '_blank');"
                                        style="display: none">
                                ...
                                @foreach ($hotelName as $item)
                                    <div class="col-lg-12 location-popup-desc-container sugest-list"
                                        ...
                                        <div class="location-popup-text sugest-list-text">
                                            <a href="{{ route('hotel', $item->id_hotel) }}" type="button"
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6  list-listing-img">
                            <img class="lozad" loading="lazy" src="{{ LazyLoad::show() }}"
                                data-src="{{ URL::asset('/foto/hotel/' . strtolower($item->name) . '/' . $item->image) }}"
                                alt="EZV_{{ $item->image }}">
                        </div>
