						...
						<div class="col-lg-12 villa-list-desc-container">
							...
							<div class="villa-list-location">
								...
								<span style="color: #ff7400;">
									<a style="z-index: 50" href="#!" onclick="view_map('{{ $data->id_villa }}')"></i>View Maps</a>
                                </span>
							</div>
							{{--
    ...
    function test(ids) {
        ...
        ids.forEach(id => {
            var request = getData(`/villa/map/${id}`).then((data) => {
                villaLocations.push([data.name, data.latitude, data.longitude, i]);
        ...
        $.when.apply(null, promises).done(() => {
            ...
            marker = new google.maps.Marker({
                    position: new google.maps.LatLng(villaLocations[1][1], villaLocations[1][2]),
                    map: map
                });
            infowindow.setContent(villaLocations[1][0]);
