...
<script>
        ...
        if(activityLocations[i].villa_nearby.length) {
            ...
            for (let j = 0; j < activityLocations[i].villa_nearby.length; j++) {
                ...
                marker = new google.maps.Marker({
                    ...
                    map: null,
                    // label: activityLocations[i].villa_nearby[j].detail.name,
                    icon: {
        ...
        if(activityLocations[i].restaurant_nearby.length) {
            ...
            for (let j = 0; j < activityLocations[i].restaurant_nearby.length; j++) {
                ...
                marker = new google.maps.Marker({
                    ...
                    map: null,
                    // label: activityLocations[i].restaurant_nearby[j].detail.name,
                    icon: {
                    ...
                    for (let k = 0; k < secondaryMarkers[i][0].length; k++) {
                        google.maps.event.addListener(secondaryMarkers[i][0][k], 'click', (function (marker, i, k) {
                            return function () {
                                ...
                                infowindow.setContent(`<a target="_blank" href="{{ env('APP_URL') }}/things-to-do/${activityLocations[i].villa_nearby[k].detail.id_activity}">${activityLocations[i].villa_nearby[k].detail.name}</a>`);
                ...
                if(secondaryMarkers[i][1]) {
                    for (let k = 0; k < secondaryMarkers[i][1].length; k++) {
                        google.maps.event.addListener(secondaryMarkers[i][1][k], 'click', (function (marker, i, k) {
                            return function () {
                                infowindow.setContent(null);
                                infowindow.setContent(`<a target="_blank" href="{{ env('APP_URL') }}/restaurant/${activityLocations[i].restaurant_nearby[k].detail.id_restaurant}">${activityLocations[i].restaurant_nearby[k].detail.name}</a>`);
                                infowindow.open(map, marker);
                            }
    ...
    function addCustomContent(activityLocations, i) {
        ...
        let image = '';
        // if(activityLocations[i].photo.length != 0) {
        //     image = '';
        //     for (let j = 0; j < activityLocations[i].photo.length; j++) {
        //         image += `<a href="{{ env('APP_URL') }}/activity/${activityLocations[i].id_activity}" target="_blank" class="col-lg-6 grid-image-container">
        //             <img class="img-fluid grid-image" style="display: block; height: 200px;"
        //                 src="{{ URL::asset('/foto/activity/${activityLocations[i].name.toLowerCase()}/${activityLocations[i].photo[j].name}')}}"
        //                 alt="">
        //         </a>`;
        //     }
        // } else {
        ...
        //                 src="{{ URL::asset('/foto/activity/${activityLocations[i].name.toLowerCase()}/${activityLocations[i].image}')}}"
        //                 alt="">
        //         </a>`;
        //     } else {
        //         image = `<a href="{{ env('APP_URL') }}/activity/${activityLocations[i].id_activity}" target="_blank" class="col-lg-6 grid-image-container">
        ...
        if(activityLocations[i].image != null) {
            ...
            image = `<a href="{{ env('APP_URL') }}/activity/${activityLocations[i].id_activity}" target="_blank" class="col-lg-6 grid-image-container">
                <img class="img-fluid grid-image" style="display: block; height:200px;"
                    src="{{ URL::asset('/template/activity/template_profile.jpg') }}"
                    alt="">
            </a>`;
        }
        ...
        if(short_description.length > 120) {
            short_description = activityLocations[i].short_description.substring(120)+'...';
        }
        ...
        var customContent = `
                            <div class="card col-12">
                                ...
                                <div class="card-body map-description">
                                    <a href="{{ env('APP_URL') }}/activity/${activityLocations[i].id_activity}" target="_blank">
                                        <p class="card-text text-secondary">
                                            <small>
                                                ${review}
                                            </small>
                                        </p>
                                        <p class="card-text object-type">${activityLocations[i].location.name}</p>
    ...
    function view_maps(mapId) {
        ...
        for (let i = 0; i < activityLocations.length; i++) {
            if (activityLocations[i].id_activity == mapId) {
                ...
                // map.setCenter(new google.maps.LatLng(activityLocations[i].latitude, activityLocations[i].longitude));
                // infowindow.addListener('domready', function() {
                //     $('.js-slider-test').slick();
                // });
...
<script>
    ...
    function fetchActivitysLocation(ids) {
        var promises = [];
        ids.forEach(id => {
            var request = getData(`{{ env('APP_URL') }}/things-to-do/map/${id}`).then((data) => {
                activityLocations.push(data);
                console.log(`process fetch activity ${id}`);
            });
        ...
        $.when.apply(null, promises).done(async () => {
            ...
            await fetchRestaurantNearbyActivity(activityLocations);
            await fetchVillaNearbyActivity(activityLocations);
        });
    }

    function fetchRestaurantNearbyActivity(activitys) {
        var promises = [];
        activitys.forEach((activity) => {
            var request = getData(`{{ env('APP_URL') }}/things-to-do/restaurant-nearby/${activity.id_activity}`).then((restaurants) => {
                activity['restaurant_nearby'] = restaurants;
                console.log(`process fetch restaurant nearby ${activity.id_activity}`);
            });
            promises.push(request);
        });
    ...
    function fetchVillaNearbyActivity(activitys) {
        var promises = [];
        activitys.forEach((activity) => {
            var request = getData(`{{ env('APP_URL') }}/things-to-do/villa-nearby/${activity.id_activity}`).then((villas) => {
                activity['villa_nearby'] = villas;
                console.log(`process fetch villa nearby ${activity.id_activity}`);
            });
            promises.push(request);
        });
