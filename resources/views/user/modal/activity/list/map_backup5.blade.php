...
<script>
    ...
    function addCustomContentActivity(activityLocations) {
        ...
        if(activityLocations.facilities.length > 4) {
            facilities = '';
            for (let j = 0; j < 4; j++) {
                var data = `${activityLocations.facilities[j].name} `;
                facilities+=data;
            }
        } else if(activityLocations.facilities.length > 0 && activityLocations.facilities.length <= 4) {
            facilities = '';
            for (let j = 0; j < activityLocations.facilities.length; j++) {
                var data = `${activityLocations.facilities[j].name} `;
                facilities+=data;
            }
        }
                            ...
                            <div class="card col-12">
                                ...
                                <div class="mt-3">
                                    <a href="{{ env('APP_URL') }}/things-to-do/${activityLocations.id_activity}" target="_blank">
                                        ...
                                        <p class="card-text text-20 text-orange fw-600 mt-1">${activityLocations.name}</p>
        ...
        for (let i = 0; i < activityLocations.length; i++) {
            ...
            google.maps.event.addListener(markerActivity[i], 'click', (function (marker, i) {
                return function () {
                    ...
                    primaryContent = addCustomContentActivity(activityLocations[i]);
    ...
    function markedViewedMarker() {
        if(viewedMarkers.length > 0 && activityLocations.length > 0 && markerActivity.length > 0) {
            viewedMarkers.forEach(id_activity => {
                for (let j = 0; j < markerActivity.length; j++) {
                    if(activityLocations[j].id_activity
                         == id_activity) {
