...
<div class="modal fade" id="modalSubWow" tabindex="-1" role="dialog" aria-labelledby="modal-default-fadein"
    aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document" style="overflow-y: initial !important">
        <div class="modal-content" style="background: #fff;">
            <div class="modal-header filter-modal">
                <div class="col-10">
                    <h5 class="mb-0">
                        {{ Translate::translate('Sub Category') }}</h5>
                </div>
                            ...
                            @foreach ($subCategory as $item)
                                <div class="col-6 checkdesign-gap">
                                    <label class="checkdesign checkdesign-modal-filter">{{ $item->name }}
                                        <input type="checkbox" name="subCategory" value="{{ $item->id_subcategory }}"
                                            onchange="subCategoryFilter({{ $item->id_subcategory }})"
                                            @if (request()->get('fSubCategory') == $item->id_subcategory) checked @endif>
