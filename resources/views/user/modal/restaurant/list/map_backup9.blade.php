    ...
    function initViewMap(){
        ...
        for (let i = 0; i < restaurantLocations.length; i++) {
            ...
            google.maps.event.addListener(markers[i], 'click', (function (marker, i) {
                return function () {
                    // mark as has been viewed marker
                    addViewedMarker(i);
...
<script>
    ...
    function fetchRestaurantsLocation(ids) {
        ...
        $.when.apply(null, promises).done(async () => {
            ...
            await fetchHotelNearbyRestaurant(restaurantLocations);
    ...
    function fetchHotelNearbyRestaurant(restaurants) {
        var promises = [];
        restaurants.forEach((restaurant) => {
            var request = getData(`{{ env('APP_URL') }}/restaurant/hotel-nearby/${restaurant.id_restaurant}`).then((hotel) => {
                restaurant['hotel_nearby'] = hotel;
                console.log(`process fetch hotel nearby ${restaurant.id_restaurant}`);
            });
            promises.push(request);
        });
