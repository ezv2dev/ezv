...
<script>
                    ...
                    for (let k = 0; k < secondaryMarkers[i][0].length; k++) {
                        google.maps.event.addListener(secondaryMarkers[i][0][k], 'click', (function (marker, i, k) {
                            return function () {
                                ...
                                infowindow.setContent(`<a target="_blank" href="{{ env('APP_URL') }}/things-to-do/${villaLocations[i].activity_nearby[k].detail.id_activity}">${villaLocations[i].activity_nearby[k].detail.name}</a>`);
                ...
                if(secondaryMarkers[i][1]) {
                    for (let k = 0; k < secondaryMarkers[i][1].length; k++) {
                        google.maps.event.addListener(secondaryMarkers[i][1][k], 'click', (function (marker, i, k) {
                            return function () {
                                infowindow.setContent(null);
                                infowindow.setContent(`<a target="_blank" href="{{ env('APP_URL') }}/restaurant/${villaLocations[i].restaurant_nearby[k].detail.id_restaurant}">${villaLocations[i].restaurant_nearby[k].detail.name}</a>`);
                                infowindow.open(map, marker);
                            }
    ...
    function addCustomContent(villaLocations, i) {
        ...
        if(short_description.length > 120) {
            short_description = villaLocations[i].short_description.substring(120)+'...';
        }
    ...
    function initViewMap(){
        ...
        for (let i = 0; i < villaLocations.length; i++) {
            addMarker({
                ...
                text: 'IDR '+villaLocations[i].price.toLocaleString() ?? 'IDR 0',
                ...
                return function () {
                    ...
                    for (let index = 0; index < markers.length; index++) {
                        markers[index].setLabel({
                            text: 'IDR '+villaLocations[index].price.toLocaleString(),
                            className: 'text-black fw-bold px-1 py-1 rounded-pill bg-white'
                        });
                    }
