...
<script>
    ...
    var infowindows = [];
    var customContents = [];
    var customContentRestaurant = [];
    var customContentActivity = [];
    var customContentHotel = [];
    var viewedMarkers = [];
    var markers = [];
            ...
            if(data) {
                ...
                if(secondaryMarkers[i][0]) {
                    for (let k = 0; k < secondaryMarkers[i][0].length; k++) {
                        ...
                        customContentRestaurant.push(content);
                        google.maps.event.addListener(secondaryMarkers[i][0][k], 'click', (function (marker, i, k) {
                            return function () {
                                infowindow.setContent(null);
                                infowindow.setContent(customContentRestaurant[k]);
                ...
                if(secondaryMarkers[i][1]) {
                    for (let k = 0; k < secondaryMarkers[i][1].length; k++) {
                        ...
                        customContentActivity.push(content);
                        google.maps.event.addListener(secondaryMarkers[i][1][k], 'click', (function (marker, i, k) {
                            return function () {
                                infowindow.setContent(null);
                                infowindow.setContent(customContentActivity[k]);
                                google.maps.InfoWindow.prototype.opened = true;
                                infowindow.open(map, marker);
                            }
    ...
    function addCustomContentActivity(activityContent) {
        ...
        if(activityContent.image != null) {
            image = `<a href="{{ env('APP_URL') }}/things-to-do/${activityContent.id_activity}" target="_blank" class="col-lg-6 grid-image-container">
                <img class="img-fluid grid-image h-180"
                    src="{{ URL::asset('/foto/activity/${activityContent.name.toLowerCase()}/${activityContent.image}')}}"
                    alt="">
            </a>`;
        } else {
            image = `<a href="{{ env('APP_URL') }}/things-to-do/${activityContent.id_activity}" target="_blank" class="col-lg-6 grid-image-container">
    ...
    function addCustomContent(villaLocations, i) {
        ...
        let price = `IDR ${villaLocations[i].price.toLocaleString() ?? 'there is no price yet'}`;
    ...
    function initViewMap(){
        ...
        for (let i = 0; i < villaLocations.length; i++) {
            addMarker({
                ...
                text: 'IDR '+villaLocations[i].price.toLocaleString() ?? 'there is no price yet',
            ...
            google.maps.event.addListener(markers[i], 'click', (function (marker, i) {
                return function () {
                    ...
                    for (let index = 0; index < markers.length; index++) {
                        markers[index].setLabel({
                            text: 'IDR '+villaLocations[index].price.toLocaleString() ?? 'there is no price yet',
                            className: 'text-black fw-bold px-1 py-1 rounded-pill bg-white'
                        });
                    }
...
<script>
    ...
    function fetchVillasLocation(ids) {
        ...
        $.when.apply(null, promises).done(async () => {
            ...
            await fetchHotelNearbyVilla(villaLocations);
    ...
    function fetchHotelNearbyVilla(villas) {
        var promises = [];
        villas.forEach((villa) => {
            var request = getData(`{{ env('APP_URL') }}/villa/hotel-nearby/${villa.id_villa}`).then((hotel) => {
                villa['hotel_nearby'] = hotel;
                console.log(`process fetch hotel nearby ${villa.id_villa}`);
            });
            promises.push(request);
        });
    ...
    function fetchRestaurantNearbyVilla(villas) {
        var promises = [];
        villas.forEach((villa) => {
            var request = getData(`{{ env('APP_URL') }}/villa/restaurant-nearby/${villa.id_villa}`).then((restaurant) => {
                villa['restaurant_nearby'] = restaurant;
