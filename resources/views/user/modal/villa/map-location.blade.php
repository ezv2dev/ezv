...
<script>
        ...
        var customContent = `
                            <div class="col-12 flex-row flex-lg-column mobile-map-desc-container" style="position: relative">
                                <div style="z-index: 43;" class="modal-map-image-container">
                                    @guest
                                        <div
                                            style="position: absolute; z-index: 43; left: 10px; top: 10px; display: flex; font-size: 24px; border-radius: 9px;">
                                            <a style="position: absolute; top: 10px; left: 10px; cursor: pointer;"
                                                onclick="loginForm(1)">
                                ...
                                <div class="map-desc-mobile col-lg-12">
                                    <a href="${contentLink}" target="_blank">
                                        ...
                                        <div class="">
                                            ...
                                            <p class="card-text text-grey-2 text-12 d-none d-sm-block fw-500 text-align-justify mt-1">${short_description}</p>
                                        </div>
                                    </a>
                                    <div class="col-12 d-flex">
                                        <div class="col-6">
    ...
    function declareMarkerVilla() {
        for (let i = 0; i < villaLocations.length; i++) {
            ...
            lat_primary = villaLocations[i].latitude;
            log_primary = villaLocations[i].longitude;
            ...
            google.maps.event.addListener(markerVilla[i], 'click', (function (marker, i) {
                return function () {
                    ...
                    resetPrimaryMarker();
                    // set primary marker
                    setPrimaryMarker(villaLocations[i]);
...
<script>
    ...
    async function fetchVillasLocation(data) {
        const response = await fetch(`/map/by-coordinate-area/villa/{{ $villa[0]->id_villa }}`)
        datas = await response.json();
        villaLocations = [];
        villaLocations.push(datas);
        declareMarkerVilla();
        // marked viewed marker villa
        markedViewedMarker();
