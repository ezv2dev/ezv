...
@section('content')
    <div style="margin-top: 300px">
        <form action="{{ route('search_activity') }}" method="post">
            @csrf
            <input type="text" class="bg-primary text-white" name="location">
        </form>
    </div>
    <div>
        <div id="category-form">
            <select name="id_category" class="category-form-input">
                <option value="">...</option>
                @forelse ($categories as $category)
                <option value="{{ $category->id_category }}" {{ request()->get('category') == $category->id_category ? 'selected':'' }}>{{ $category->name }}</option>
                @empty
                    {{--  --}}
                @endforelse
            </select>
        ...
        <div id="subcategory-form">
            @forelse ($subcategories as $subcategory)
                @php
                    $isChecked = '';
                    $subcategoryIds = explode(',', request()->get('subcategory'));
                    if(in_array($subcategory->id_subcategory, $subcategoryIds)){
                        $isChecked = 'checked';
                    }
                @endphp
                <div id="subcategory-form-input{{ $subcategory->id_subcategory  }}" style="display: none">
                    <input type="checkbox" class="subcategory-form-input" name="id_subcategory[]" id="{{ $subcategory->id_subcategory  }}" value="{{ $subcategory->id_subcategory  }}" {{ $isChecked }}>
                    <label for="{{ $subcategory->id_subcategory  }}">{{ $subcategory->name }}</label>
                </div>
            @empty
            @endforelse
        </div>
        <div id="facilities-form">
            @forelse ($facilities as $facility)
                @php
                    $isChecked = '';
                    $facilitiesIds = explode(',', request()->get('facilities'));
                    if(in_array($facility->id_facilities, $facilitiesIds)){
                        $isChecked = 'checked';
                    }
                @endphp
                <input type="checkbox" class="facilities-form-input" name="id_facilities[]" value="{{ $facility->id_facilities }}" {{ $isChecked }}>{{ $facility->name }}</input>
            @empty
                {{--  --}}
            @endforelse
        </div>
    ...
    @section('scripts')
        ...
        <script>
            function activityRefreshTest(suburl){
                window.location.href = `http://localhost:8000/filteractivity?${suburl}`;
            }
        </script>

        <script>
            var categoryFormInput = $('#category-form').children('.category-form-input');
            // var subcategoryFormInput = $('#subcategory-form').children('.subcategory-form-input');
            var subcategoryFormInput = [];
            ...
            $(window).on("load", async () => {
                ...
                if(url.includes("subcategory=") && url.includes("category=")) {
                    var categoryValue = url.split('&')[0].split('?')[1].split('=')[1];
                    await fetch(`http://localhost:8000/filteractivity/subcategory?category=${categoryValue}`)
                    .then((response) => {
                        return response.json();
                    })
                    .then((datas) => {
                        datas.map((data) => {
                            console.log(data);
                            console.log($('#subcategory-form').children('1'));
                            $('#subcategory-form').children(`#subcategory-form-input${data.id_subcategory}`).show();
                        });
                    });
                }
            ...
            $('#category-form').change(async (event) => {
                // console.log($(categoryFormInput).val());

                subcategoryFormInput = [];
                $("input[name='id_subcategory[]']:checked").each(function ()
                {
                    subcategoryFormInput.push(parseInt($(this).val()));
                });
                // console.log(subcategoryFormInput);
                ...
                var subUrl= `category=${$(categoryFormInput).val()}&subcategory=${subcategoryFormInput}&facilities=${facilitiesFormInput}`;
                console.log(subUrl);
                activityRefreshTest(subUrl);
            });

            $('#subcategory-form').change(async (event) => {
                // test value of sub category form
                subcategoryFormInput = [];
                $("input[name='id_subcategory[]']:checked").each(function ()
                {
                    subcategoryFormInput.push(parseInt($(this).val()));
            ...
            $('#facilities-form').change(async (event) => {
                console.log('changes facilities');
                // test value of sub category form
                subcategoryFormInput = [];
                $("input[name='id_subcategory[]']:checked").each(function ()
                {
