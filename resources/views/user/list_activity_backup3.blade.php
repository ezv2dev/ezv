...
@section('content')
    ...
    <div>
        ...
        <div id="subcategory-form"></div>
        <div id="facilities-form">
            @forelse ($facilities as $facility)
                <input type="checkbox" class="facilities-form-input" name="id_facilities[]" value="{{ $facility->id_facilities }}">{{ $facility->name }}</input>
            @empty
                {{--  --}}
            @endforelse
        </div>
    ...
    <div id="activity-loading" style="display: none;">
        <div class="w-100 d-flex justify-content-center align-items-center" style="height: 200px;">
            <img src="https://c.tenor.com/5o2p0tH5LFQAAAAj/hug.gif" class="img-fluid">
        </div>
    </div>
                    ...
                    @foreach ($activity as $data)
                        ...
                        <div class="row top-20">
                            ...
                            <p class="promo"><a type="button" class="btn-promo btn-sm"><i class="fa fa-gift"></i> Promotion</a></p> --}}
                                        {{-- if promotion false --}}
                                        <p class="promo" data-bs-toggle="popover" data-bs-animation="true" data-bs-placement="bottom"
                            ...
                            <div class="col-lg-3 col-md-3 col-sm-12">
                                <div class="col-sm-12 rates-block">
                                    ...
                                    <div style="padding-top:10px">
                                        <a type="button" href="{{ route('activity', $data->id_activity) }}" class="btn btn-sm btn-choose"
                                            target="_blank">Choose Activity</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- AMENITIES -->
                        @if (!$data->facilities->isEmpty())
                            <div class="modal fade" id="modal-amenities-{{ $data->id_activity }}" tabindex="-1" role="dialog"
...
@section('scripts')
    ...
    <script>
        ...
        var load = true;
        var error = '';
        // add sub category when category selected
        $('#category-form').change(async (event) => {
            // fetch all activity related to subcategory, & append to tag
            await fetch(`http://localhost:8000/filteractivity/subcategory?category=${$(categoryFormInput).val()}`)
                .then((response) => {
                ...
                .then((datas) => {
                    ...
                    datas.map((data) => {
                        $('#subcategory-form').append(`<input type="checkbox" class="subcategory-form-input" name="id_subcategory[]" value="${data.id_subcategory}">${data.name}</input>`);
                    });
                });

            // test value of category form
            ...
            console.log(subUrl);
            fetch(`http://localhost:8000/filteractivity?${subUrl}`)
                .then((response) => {
                    return response.json();
                })
                .then((datas) => {
        ...
        $(window).on("load", async () => {
            await fetch('http://localhost:8000/filteractivity')
                ...
                .then((datas) => {
                    ...
                    $('#activity-test').show();
                    this.datas.map((data) => {
                        console.log(data.facilities.length);
                    });
                    let appenedHtml = `
                        @foreach (json_decode('<script>datas</script>') as $data)
                            <input type='hidden' value='{{ $data->id_activity }}' id='id_activity' name='id_activity'>
                            <div class='row top-20'>
                                <div class='col-lg-4 col-md-3 col-sm-12'>
                                    <div class='js-slider slick-nav-black slick-dotted-inner slick-dotted-white' data-dots='false'
                        ...
                        @endforeach
                    `;
                    $('#activity-test').append(appenedHtml);
